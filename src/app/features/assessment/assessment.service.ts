...
export class AssessmentService {
  ...
  startAttempt(assessmentId: string) {
    return this.http.post<{attempt_id: string, start_time: string, end_time: string}>(`${this.apiUrl}/start/${assessmentId}`, {}, { withCredentials: true });
  }
  ...
  submitAnswers(attemptId: string, payload: { answers: any[], violations: number }) {
    return this.http.post(`${this.apiUrl}/attempt/${attemptId}/submit`, payload, { withCredentials: true });
  }
}
